...
function simulateArenaBattle(player, opponent) {
  ...
  const oAtk = opponent.attack;
  const oDef = opponent.defense;
  const oMhp = opponent.maxHp;
  ...
  let round = 0;
  let bleed = 0;
  let totalDmgDealt = 0, totalDmgTaken = 0, crits = 0;

  log.push({txt:`⚔️ 競技場對決！`,type:"title"});
  log.push({txt:`你 (Lv.${player.level} 攻${pAtk} 防${pDef}) vs ${opponent.name} (Lv.${opponent.level} 攻${oAtk} 防${oDef})`,type:"info"});
  ...
  const playerFirst = pSpd >= oSpd || (wc&&wc.trait==="firstblood");

  if(playerFirst) log.push({txt:`⚡ 你先手！（速度 ${pSpd} vs ${oSpd}）`,type:"hit"});
  else            log.push({txt:`⚡ 對手先手！（速度 ${oSpd} vs ${pSpd}）`,type:"enemy"});

  const bleedRef = {val:0};
  // Build a fake "enemy" object for fightMonster compatibility
  const fakeEnemy = {
    name: opponent.name, icon:"🏟",
    hp: oHp, maxHp: oMhp,
    attack: oAtk, defense: oDef,
  ...
  // Use the existing fightMonster function
  const np = {...player, hp: pHp};
  const result = fightMonster(fakeEnemy, np, pAtk, pDef, pMhp, specials, wc, log, bleedRef);

  const won = result.won;
  let goldPlundered = 0;

  if(won) {
    const plunderPct = 0.10 + Math.random()*0.15; // 10-25%
    goldPlundered = Math.floor(opponent.goldCarried * plunderPct);
    log.push({txt:`━━━━━━━━━━━━━━━━━━`,type:"sep"});
