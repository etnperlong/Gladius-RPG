...
const WEAPON_CATEGORIES = {
  ...
  dagger:   { label:"匕首",   icon:"🗡️",  trait:"swift",      traitDesc:"速度增加3，首回合傷害1.5倍"         },
  ...
  sickle:   { label:"鐮刀",   icon:"☽",   trait:"crit_boost", traitDesc:"爆擊率額外增加 10%"               },
  angel:    { label:"死亡天使",icon:"🪽",  trait:"soulstrike",  traitDesc:"低血量時傷害1.5倍（敵人生命<30%）"  },
...
const TRAIN_STATS = [
  ...
  { id:"trainedHp",  label:"生命值", icon:"❤️", color:"#c84040", desc:"永久提升最大生命值",  hpStat:true },
...
const MERC_BASES = [
  ...
  { id:"healer",   name:"精靈治癒師", icon:"🧝", attack:5,  defense:4,  hp:50,  heal:8, desc:"每回合回復 8生命值",   grade:"mythic"    },
  { id:"berserker",name:"狂戰士",     icon:"🪖", attack:25, defense:5,  hp:80,  heal:0, desc:"狂暴：攻擊增加30%",   grade:"legendary" },
  { id:"paladin",  name:"聖騎士",     icon:"⚜️", attack:12, defense:15, hp:120, heal:5, desc:"聖護：防禦加回血",  grade:"mythic"    },
  { id:"assassin", name:"暗殺者",     icon:"🌑", attack:30, defense:2,  hp:30,  heal:0, desc:"暗殺：首攻2倍傷害",     grade:"mythic"    },
];
...
const MONSTERS = {
  ...
  boar:       { name:"野豬",     icon:"🐗", hpM:1.1,  atkM:1.0,  defM:0.8,  trait:"charge",   traitDesc:"第一擊傷害增加30%",   lore:"憤怒衝鋒的野豬"               },
  bandit:     { name:"山賊",     icon:"🗡️", hpM:0.9,  atkM:1.1,  defM:0.7,  trait:"dodge",    traitDesc:"20%閃避",          lore:"亡命之徒，出手狡猾"           },
  wolfKing:   { name:"狼王",     icon:"🐺", hpM:2.5,  atkM:1.8,  defM:1.2,  trait:"howl",     traitDesc:"戰吼：攻擊增加20%",   lore:"統領狼群的古老王者", boss:true },
  goblin:     { name:"地精",     icon:"👺", hpM:0.7,  atkM:1.0,  defM:0.6,  trait:"swarm",    traitDesc:"成群時攻擊增加15%",   lore:"矮小貪婪的礦坑居民"           },
  ghostMiner: { name:"礦工亡魂", icon:"👻", hpM:0.8,  atkM:1.2,  defM:0.3,  trait:"phase",    traitDesc:"無視25%防禦",      lore:"死於坑難的礦工怨靈"           },
  golem:      { name:"石巨人",   icon:"🪨", hpM:1.8,  atkM:0.9,  defM:1.5,  trait:"armor",    traitDesc:"防禦增加50%",         lore:"礦石能量凝聚而成的巨型石人"   },
  ...
  cultist:    { name:"黑暗祭司", icon:"🧎", hpM:0.8,  atkM:1.3,  defM:0.5,  trait:"curse",    traitDesc:"詛咒：攻擊減少10%",   lore:"供奉黑暗神明的祭司"           },
  ...
  dragonKnight:{name:"龍族武士", icon:"⚔️", hpM:1.5,  atkM:1.6,  defM:1.3,  trait:"dragonArmor",traitDesc:"鱗甲：防禦增加40%",lore:"古龍麾下最強戰士"             },
  ancientDragon:{name:"古龍",    icon:"🐉", hpM:4.0,  atkM:2.5,  defM:2.0,  trait:"dragonRage",traitDesc:"低血爆怒增加60%傷害",lore:"千年古龍，大地之王", boss:true },
  shadowBeast:{ name:"暗影獸",   icon:"🌑", hpM:1.0,  atkM:1.4,  defM:0.7,  trait:"dark",     traitDesc:"黑暗：降低視野與攻擊",lore:"黑暗中游走的詭異生物"       },
  lich:       { name:"巫妖",     icon:"🧙", hpM:1.1,  atkM:1.8,  defM:0.6,  trait:"soulSuck", traitDesc:"吸魂：回復已損失生命值的5%",lore:"追求永生的邪惡術士"       },
  titan:      { name:"泰坦",     icon:"🗿", hpM:2.5,  atkM:1.4,  defM:2.2,  trait:"stonewall", traitDesc:"石牆：每回合回復5生命值",lore:"遠古時代的神秘巨人"          },
...
function applyWeaponTrait(cat, dmg, enemy, isFirstRound, bleedStacks){
  ...
  switch(cat.trait){
    ...
    case "crit_boost":
      ...
      break;
    case "soulstrike":
      if(enemy.hp<enemy.maxHp*0.3){finalDmg=Math.floor(finalDmg*1.5);log.push(`👻 【死亡天使】低血量觸發！傷害1.5倍`);}
      break;
    case "bonecrush":
      finalDmg+=2;
      break;
...
function enemyAttackPlayer(enemy, pDef, specials, np, pMhp, log, round) {
  ...
  if(enemy.regenVal>0) {
    ...
    log.push({txt:`💚 ${enemy.name} 回復 ${enemy.regenVal}生命值`,type:"enemy"});
  }
  ...
  if(enemy.trait==="dragonRage" && enemy.hp<enemy.maxHp*0.3) { baseDmg=Math.floor(baseDmg*1.6); log.push({txt:`😡 古龍暴怒！傷害1.6倍`,type:"enemy"}); }
    ...
    <div className="iaf">
      {affixes.map((a,i) => (
        <div key={i} className={`al${a.special?" as":""}`}>
          {a.stat
            ? `${a.tag}: +${a.rolledVal} ${a.stat==="attack"?"攻擊":a.stat==="defense"?"防禦":a.stat==="hp"?"生命值":"速度"}`
            : `${a.tag}: ${a.rolledVal}${
  ...
  return (
    <div className="ii" style={{
      ...
      <div className="iis">
        ...
        {item.hp>0      && <div style={{color:item.hp>80?"#f06060":item.hp>40?"#c84040":"#5a4020"}}>生命值 +{item.hp}</div>}
        {item.speed>0   && <div style={{color:"#5a9050"}}>速度 +{item.speed}</div>}
        {item.heal      && <div style={{color:"#50a860"}}>回復 {item.heal} 生命值</div>}
      </div>
      {/* Weapon trait */}
...
function HpBar({ cur, max, color="#c83030", thin }) {
  return (
    <div className="bw">
      <div className="bl"><span>生命值</span><span>{cur}/{max}</span></div>
    ...
    <div className="lp">
      <div className="lb" style={{
        ...
        <div className="lst">
          ...
          {item.hp>0      && <div>生命值 {isMercScroll?"":"+"}{item.hp}</div>}
          {item.speed>0   && <div>速度 +{item.speed}</div>}
          {item.heal>0    && <div style={{color:"#50c890"}}>每回合回復 {item.heal}生命值</div>}
...
export default function App() {
  ...
  // Derived from inventory — merc scrolls available and selected
  ...
  // Replay state — battle log animation
  ...
  function fightMonster(enemy, np, pAtk, pDef, pMhp, specials, wc, log, bleedRef) {
    ...
    while(np.hp>0&&enemy.hp>0&&round<80){
      ...
      let dmgMult=1;
      if((wc&&wc.trait)==="swift"&&firstRound){dmgMult=1.5;log.push({txt:`⚡ 先制一擊！1.5倍`,type:"hit"});}
      ...
      if(healed>0){np.hp=Math.min(np.hp+healed,pMhp);log.push({txt:`🩸 吸血+${healed}生命值`,type:"heal"});}
      const regen=specials.filter(s=>s.type==="regen"||s.type==="vampiric").reduce((a,x)=>a+x.val,0);
      if(regen>0&&np.hp>0){np.hp=Math.min(np.hp+regen,pMhp);log.push({txt:`💚 回復+${regen}生命值`,type:"heal"});}
      if(enemy.hp<=0) break;
      if(!stunned){
        ...
        if(enemy.trait==="dragonRage"&&enemy.hp<enemy.maxHp*0.3){eDmg=Math.floor(eDmg*1.6);log.push({txt:`😡 古龍暴怒！1.6倍`,type:"enemy"});}
        if(enemy.trait==="stonewall"){enemy.hp=Math.min(enemy.maxHp,enemy.hp+5);log.push({txt:`💚 ${enemy.name}回復5生命值`,type:"enemy"});}
  ...
  function simulateExpedition(expedition, initPlayer) {
    ...
    log.push({txt:`━━ ${enemy.icon}${enemy.name} ｜ 生命值:${enemy.maxHp} 攻:${enemy.attack} 防:${enemy.defense} ━━`,type:"sep"});
    ...
    if(r.won){
      ...
      log.push({txt:`✅ 擊敗${enemy.name}！增加 ${expG} 經驗值 增加 ${goldG} 金幣`,type:"win"});
  ...
  function simulateRun(dungeon, tier, initPlayer){
    ...
    for(const wave of dungeon.waves){
      ...
      for(const mKey of wave.monsters){
        ...
        log.push({txt:`${enemy.icon}${enemy.name} 出現！(生命值:${enemy.maxHp} 攻:${enemy.attack} ⚡${enemy.traitDesc})`,type:"info"});
        const r=fightMonster(enemy,np,pAtk,pDef,pMhp,specials,wc,log,bleedRef);
    ...
    if(np.hp>0&&dungeon.boss){
      ...
      log.push({txt:`${boss.icon}${boss.name} ｜ 生命值:${boss.maxHp} 攻:${boss.attack} 防:${boss.defense}`,type:"info"});
      log.push({txt:`👹 首領特性：${boss.traitDesc}`,type:"info"});
      ...
      if(r.won){
        ...
        np=lvUp(np,expG,goldG,log);
        log.push({txt:`👑 擊敗首領 ${boss.name}！增加 ${expG} 經驗值 增加 ${goldG} 金幣`,type:"win"});
        if(Math.random()<0.60+tier.lootBonus){const d=genLoot(np.level,tier.lootBonus+0.1);drops.push(d);log.push({txt:`✨ 首領掉落：${d.rarityLabel}【${d.name}】`,type:"loot"});}
        if(Math.random()<0.25+tier.lootBonus){const s=genMercScroll(np.level);drops.push(s);log.push({txt:`📜 首領掉落契約捲軸：${s.rarityLabel}【${s.name}】`,type:"loot"});}
      } else {
        log.push({txt:`💀 敗於首領 ${boss.name}！`,type:"lose"});
      }
    }
    const won=np.hp>0;
  ...
  function simulateMercRun(dungeonId, initPlayer, mercs){
    ...
    for(let wi=0;wi<dungeon.waves.length;wi++){
      ...
      for(const mKey of wave.enemies){
        ...
        while(np.hp>0&&enemy.hp>0&&round<60){
          ...
          if(healerMerc){const ha=healerMerc.heal||8;np.hp=Math.min(np.hp+ha,pMhp);log.push({txt:`💚${healerMerc.name}回復${ha}生命值`,type:"heal"});}
    ...
    if(np.hp>0&&dungeon.boss){
      ...
      log.push({txt:`生命值:${bossEnemy.maxHp} 攻:${bossEnemy.attack} 防:${bossEnemy.defense}`,type:"info"});
      ...
      if(r.won){
        ...
        log.push({txt:`👑 擊敗首領${bd.name}！增加 ${bossEnemy.expReward} 經驗值`,type:"win"});
          ...
          <aside className="sb">
            <div className="pn">
              ...
              <div className="pb">
                ...
                <div className="bw">
                  <div className="bl"><span>生命值</span><span>{player.hp}/{tMhp}</span></div>
                      ...
                      <div style={{minWidth:0,flex:1}}>
                        ...
                        {eq ? <>
                          ...
                          <div style={{fontSize:10,color:"#6a5030",marginTop:2,display:"flex",gap:6,flexWrap:"wrap"}}>
                            ...
                            {eq.hp>0 && <span style={{color:"#c84040"}}>生命值+{eq.hp}</span>}
                    ...
                    {EXPEDITIONS.map(exp=>{
                      ...
                      return(
                        <div key={exp.id} className={`dc${lk?" lk":""}`} onClick={()=>!lk&&startExpedition(exp)}
                          ...
                          <div className="drw">經驗值倍率: {exp.expMult} 金幣倍率: {exp.goldMult}{exp.lootBonus>0&&<span style={{color:"#4caf50"}}> +{Math.round(exp.lootBonus*100)}%</span>}</div>
                        </div>
                      );
                    })}
                  </div>
                        ...
                        {DUNGEON_TIERS.map(t=>{
                          ...
                          return(
                            <div key={t.id} className={`dc${lk?" lk":""}`} onClick={()=>!lk&&startBattle(d,t)}>
                              ...
                              <div className="drw">經驗值倍率: {t.expMult} 金幣倍率: {t.goldMult}{t.lootBonus>0&&<span style={{color:"#d4b84a"}}> +{Math.round(t.lootBonus*100)}%</span>}</div>
                            </div>
                          );
                        })}
                      </div>
                            ...
                            return(
                              <div key={scroll.uid}
                                ...
                                <div style={{fontSize:10,color:"#5a4020",marginTop:3,lineHeight:1.5}}>
                                  攻{scroll.attack} 防{scroll.defense} 生命值{scroll.hp}
                      ...
                      {MERC_DUNGEONS.map((d)=>{
                        ...
                        return(
                          <div key={d.id} className={`dc${lk?" lk":""}`} onClick={()=>!lk&&startMercBattle(d.id)}
                            ...
                            <div className="drw">經驗值倍率: {d.reward.expMult} 金幣倍率: {d.reward.goldMult}</div>
                          </div>
                        );
                      })}
                    </div>
                ...
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
                    {TRAIN_STATS.map(stat=>{
                      ...
                      const effect=stat.hpStat?`每次增加 3 最大生命值（已訓${current}次，增加 ${current*3} 生命值）`:`每次增加 1 ${stat.label}（已訓${current}次）`;
                      return(
                        <div key={stat.id} style={{
                          ...
                          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                            <div style={{fontFamily:"'Cinzel',serif",fontSize:14,color:stat.color}}>
                              ...
                              <span style={{fontSize:10,color:"#5a4020",marginLeft:4}}>（基礎增加 {current} 訓練）</span>
                            </div>
                          ...
                          <button className="btn btp" style={{width:"100%",fontSize:11}}
                            ...
                            disabled={!canAfford}>
                            {canAfford?`訓練 (花費 🪙${cost})`:`金幣不足（需 ${cost}，保留50）`}
                          </button>
                        </div>
                      );
                    })}
                  ...
                  <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:14}}>
                    {ENHANCE_LEVELS.map(l=>(
                      <div key={l.lv} style={{
                        ...
                        <div style={{fontSize:9,color:"#4caf50"}}>增加 {Math.round(l.bonus*100)}%</div>
                      </div>
                    ))}
                  </div>
                      ...
                      return(
                        <div key={item.uid} className="ii"
                          ...
                          <div className="iis">
                            ...
                            {item.hp>0&&<div style={{color:"#c84040"}}>生命值+{item.hp}</div>}
                          </div>
                          {!isMax&&lvData&&(
                  ...
                  {replay ? (
                    <>
                      <div className="btl">
                        ...
                        {!replay.isExpedition&&!replay.isMerc&&` · ${(replay.tier&&replay.tier.label)}`}
                      </div>
                    ...
                    <div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                        ...
                        <button className="btn btm" style={{fontSize:10,padding:"5px 12px"}} onClick={refreshShop}>
                          🔄 刷新 (花費 🪙{Math.floor(player.level*5+20)})
                        </button>
                      </div>
                      ...
                      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
                        {[{name:"小型回復藥",icon:"🧪",heal:30,cost:25},{name:"大型回復藥",icon:"⚗️",heal:80,cost:60}].map((p,i)=>(
                          <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:"#1a1208",border:"1px solid #3a2a10",borderRadius:4,fontSize:12}}>
                            ...
                            <span style={{color:"#50a860"}}>+{p.heal}生命值</span>
                              ...
                              <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                                ...
                                <div style={{flex:1}}>
                                  ...
                                  <div style={{fontSize:11,color:"#6a5030",display:"flex",gap:8,flexWrap:"wrap",marginBottom:4}}>
                                    ...
                                    {it.hp>0&&<span style={{color:"#c84040"}}>生命值+{it.hp}</span>}
                          ...
                          if(item.type==="merc_scroll"){
                            return(
                              <div key={item.uid} className="ii" style={{borderColor:rar.color+"55",background:`linear-gradient(160deg,${rar.color}08,#120e06)`}}>
                                ...
                                <div className="rb" style={{color:rar.color,borderColor:rar.color+"55",background:`${rar.color}15`}}>{rar.label}</div>
                                <div className="iin" style={{color:rar.color}}>{item.name}</div>
                                <div className="iis">攻{item.attack} 防{item.defense} 生命值{item.hp}</div>
                    ...
                    {filteredInv.map(item=>{
                      if(item.type==="potion") return(
                        <div key={item.uid} className="ii">
                          ...
                          <div className="iis" style={{color:"#50a860"}}>回復 {item.heal}生命值</div>
                        ...
                        return(
                          <div key={item.uid} className="ii" style={{borderColor:gr.color+"66",background:`linear-gradient(160deg,${gr.color}10,#120e06)`,boxShadow:gr.glow||"none"}}>
                            ...
                            <div className="iis">
                              ...
                              <div style={{color:"#c84040"}}>生命值 {item.hp}</div>
                              {item.heal>0&&<div style={{color:"#50c890"}}>回復 {item.heal}/回</div>}
                            </div>
                            {item.affixes&&item.affixes.length>0&&<div className="iaf">
                              {item.affixes.map((a,i)=>(
                                <div key={i} className={`al${a.special?" as":""}`}>
                                  {a.stat?`${a.tag}:+${a.rolledVal}`:a.special==="all"?`${a.tag}:全屬+${Math.round(a.rolledVal*100)}%`:a.special==="first"?`${a.tag}:先攻倍率 ${1+a.rolledVal}`:""}
                                </div>
                              ))}
                            </div>}
